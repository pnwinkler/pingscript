...
error_dest = "/home/philip/Desktop/pingscript_ERROR"
pingcount = 5
...
def file_exists(dest):
    # performance: import here, only if needed.
    import os
...
def return_leading_lines(dest):
    # cache leading lines
    # islice performs better than a typical read for large files
        ...
        if does_file_exist:
            ...
            for line in leading_lines:
                try:
                    ...
                    # this has never happened. It remains here as a precaution.
                    # Here we print so any users terminal-testing the program know
                    # we write to error_dest in case this as-yet-unseen error did not occur during testing.
                    print("ERROR: unexpected exception\n", e)
                    with open(error_dest, 'a+') as f:
                        f.write(str(now) + '\t' + str(e) + '\n')
                    continue
                if found_date:
...
def archive_file(source, archive_folder):
    ...
    backup_basename = 'pingtimes-' + last_month.strftime('%b').upper() + last_month.strftime('%Y')
...
def write(dest, VPN_active, prepend_abstract=False):
    # takes string dest, bool VPN_active, bool prepend_abstract
    # writes to destination specified by dest
    if prepend_abstract:
